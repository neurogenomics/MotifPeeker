...
sequences recognised by the transcription factors, called motifs. *MotifPeeker*
...
- TIP-Seq alignment file (`CTCF_TIP_alignment.bam`) was manually processed using
  the [`nf-core/cutandrun`](https://nf-co.re/cutandrun/3.2.2) pipeline. The raw
  read files were sourced from *NIH Sequence Read Archives*
...
```R
## MACS2/3 peak files
peak_files <- list("/path/to/peak1.narrowPeak",
                   "/path/to/peak2.narrowPeak")
                   
## or SEACR peak files
peak_files <- list("/path/to/peak1.bed",
                   "/path/to/peak2.bed")
```
...
```R
## JASPAR motif files
peak_files <- list("/path/to/motif1.jaspar",
                   "/path/to/motif2.jaspar")
```
