...
[![](https://img.shields.io/badge/devel%20version-1.5.1-black.svg)](https://github.com/neurogenomics/MotifPeeker)
...
[![](https://codecov.io/gh/neurogenomics/MotifPeeker/branch/master/graphs/icicle.svg "Codecov icicle graph")](https://app.codecov.io/gh/neurogenomics/MotifPeeker/tree/master)
...
[`library`](https://rdrr.io/r/base/library.html)`(`[`MotifPeeker`](https://github.com/neurogenomics/MotifPeeker)`)`` `[`data`](https://rdrr.io/r/utils/data.html)`(``"CTCF_ChIP_peaks"``, package ``=`` ``"MotifPeeker"``)`` `[`data`](https://rdrr.io/r/utils/data.html)`(``"CTCF_TIP_peaks"``, package ``=`` ``"MotifPeeker"``)`` `[`data`](https://rdrr.io/r/utils/data.html)`(``"motif_MA1102.2"``, package ``=`` ``"MotifPeeker"``)`` `[`data`](https://rdrr.io/r/utils/data.html)`(``"motif_MA1930.2"``, package ``=`` ``"MotifPeeker"``)`

Prepare input files.

`peak_files`` ``<-`` `[`list`](https://rdrr.io/r/base/list.html)`(``CTCF_ChIP_peaks``, ``CTCF_TIP_peaks``)`` ``alignment_files`` ``<-`` `[`list`](https://rdrr.io/r/base/list.html)`(`` `` `[`system.file`](https://rdrr.io/r/base/system.file.html)`(``"extdata"``, ``"CTCF_ChIP_alignment.bam"``, package ``=`` ``"MotifPeeker"``)``,`` `` `[`system.file`](https://rdrr.io/r/base/system.file.html)`(``"extdata"``, ``"CTCF_TIP_alignment.bam"``, package ``=`` ``"MotifPeeker"``)`` ``)`` ``motif_files`` ``<-`` `[`list`](https://rdrr.io/r/base/list.html)`(``motif_MA1102.2``, ``motif_MA1930.2``)`

Run
[`MotifPeeker()`](https://neurogenomics.github.io/MotifPeeker/reference/MotifPeeker.md):

[`MotifPeeker`](https://neurogenomics.github.io/MotifPeeker/reference/MotifPeeker.md)`(`` `` peak_files ``=`` ``peak_files``,`` `` reference_index ``=`` ``2``, ``# Set TIP-seq experiment as reference`` `` alignment_files ``=`` ``alignment_files``,`` `` exp_labels ``=`` `[`c`](https://rdrr.io/r/base/c.html)`(``"ChIP"``, ``"TIP"``)``,`` `` exp_type ``=`` `[`c`](https://rdrr.io/r/base/c.html)`(``"chipseq"``, ``"tipseq"``)``,`` `` genome_build ``=`` ``"hg38"``,`` `` motif_files ``=`` ``motif_files``,`` `` cell_counts ``=`` ``NULL``, ``# No cell-count information`` `` motif_discovery ``=`` ``TRUE``,`` `` motif_discovery_count ``=`` ``3``,`` `` motif_db ``=`` ``NULL``,`` `` download_buttons ``=`` ``TRUE``,`` `` out_dir ``=`` `[`tempdir`](https://rdrr.io/r/base/tempfile.html)`(``)``,`` `` BPPARAM ``=`` ``BiocParallel``::`[`MulticoreParam`](https://rdrr.io/pkg/BiocParallel/man/MulticoreParam-class.html)`(``2``)``, ``# Use 2 CPU cores`` `` debug ``=`` ``FALSE``,`` `` quiet ``=`` ``FALSE``,`` `` verbose ``=`` ``TRUE`` ``)`
...
## [1] rworkflows_1.0.12 rmarkdown_2.32   
## 
## loaded via a namespace (and not attached):
##  [1] gtable_0.3.6        jsonlite_2.0.0      renv_1.2.4         
##  [4] dplyr_1.2.1         compiler_4.6.1      BiocManager_1.30.27
...
## [16] knitr_1.52          yulab.utils_0.2.5   tibble_3.3.1       
## [19] desc_1.4.3          dlstats_0.1.8       rprojroot_2.1.1    
## [22] pillar_1.11.1       RColorBrewer_1.1-3  rlang_1.3.0        
## [25] badger_0.2.5        xfun_0.60           fs_2.1.0           
...
## [40] farver_2.1.2        tools_4.6.1         pkgconfig_2.0.3    
## [43] htmltools_0.5.9
```
