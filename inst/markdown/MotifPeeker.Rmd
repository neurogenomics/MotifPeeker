...
peak_files_encode <- Vectorize(MotifPeeker:::check_ENCODE, "encode_id")(
    params$peak_files, expect_format = c("narrowPeak", "bed"))
alignment_files_encode <- Vectorize(MotifPeeker:::check_ENCODE, "encode_id")(
...
result <- list(
    ...
    exp_type = unname(Vectorize(MotifPeeker:::format_exptype, "exp_type")(exp_types))
)
...
if (comparison_metrics) {
    ...
    segregated_peaks <- lapply(comparison_indices, function(x) {
        MotifPeeker:::segregate_seqs(result$peaks[[params$reference_index]],
                        result$peaks[[x]])
    })
    
    ## Calculate enrichment for segregated peaks
    if (user_motif_metrics) {
      enrichment_df <- MotifPeeker:::get_df_enrichment(
