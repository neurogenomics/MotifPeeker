...
check_emo <- ifelse(requireNamespace("emoji", quietly = TRUE),
                emoji::emoji("check"), "*")
...
motif_summit_dist_bootstrap_df <- if (params$distance_bootstrap) {
    get_df_distances_bootstrapped(
        result, user_motifs, genome_build,
        samples_n = params$bootstrap_n,
        samples_len = params$bootstrap_len,
...
## Input Datasets {-}
`MotifPeeker` was run with the following input datasets:

```{r echo = FALSE}
input_summary <- dplyr::tibble(
    "Index" = seq_along(result$peaks),
    "Reference" = ifelse(seq_along(result$peaks) == params$reference_index, check_emo, ""),
    "Dataset Label" = result$exp_labels,
    "Experiment Type" = result$exp_type
)
if (!is.null(params$cell_counts)) {
    input_summary$`Cell Count` <- params$cell_counts
}
input_summary %>% print_DT(class="compact hover stripe", rownames = FALSE)
```
...
> A *smaller* absolute mean/median motif-summit distance means the detected
signal is more tightly centred on the motif, which usually reflects higher
precision and quality in epigenomic profiling experiments.  
...
if (!user_motif_metrics) {
    ...
    for (motif_i in seq_len(motif_len)) {
        ...
        cat("\n#### By Experiment Type {- .unlisted}  \n")
        cat("The graph shows how many peaks occur at different distances from",
            "the motif centre. The probability density curve shows where peaks",
            "are most common, higher areas mean peaks are more likely at those",
            "distances. The area under each curve adds up to 1, and different",
            "colours show experiment types.")
        ## Limits
        ...
        ## Density plot
        motif_summit_dist_plt <- motif_summit_dist_df %>%
            filter(motif_indice == motif_i) %>%
            ...
            labs(color = "Experiment Type",
                ...
                y = "Density")
        motif_summit_dist_plt <- MotifPeeker:::to_plotly(motif_summit_dist_plt,
                                            html_tags = FALSE)
        
        print(htmltools::tagList(motif_summit_dist_plt))
        ...
        if (!distance_bootstrap) {
            ...
            cat("This plot shows the bootstrapped distribution of the mean absolute distances of motif centre to peak summits. Larger frequencies at smaller distances indicate that the experiment consistently shows peaks close to the motif centre.  \n")
            cat("Each experiment was resampled", max(motif_summit_dist_bootstrap_df$bootstrap_iteration), "times. \n")
            
            ## Distance distribution histogram
            motif_summit_dist_bootstrap_plt <- motif_summit_dist_bootstrap_df %>%
                filter(motif_indice == motif_i) %>%
                group_by(exp_label, exp_type) %>%
                ggplot(aes(
                    x = distance,
                ...
                geom_histogram(bins = 30,
                               position = "identity",
                               alpha = 0.5) +
                labs(x = "Absolute Mean Distance to Motif Centre (bp)",
                     y = "Frequency of Resamples",
                     fill = "Experiment Label") +
                scale_fill_viridis(
                    begin = 0.15,
                    end = 0.6,
                    discrete = TRUE,
            ...
            print(MotifPeeker:::to_plotly(motif_summit_dist_bootstrap_plt, html_tags = TRUE))
        }
...
user-provided motif in the peaks was determined using the
[AME](https://meme-suite.org/meme/doc/ame.html) tool from the MEME suite.  
...
The AME tool (Analysis of Motif Enrichment) from the MEME Suite tests whether a
given transcription factor-binding motif (TFBM) appears more often in your set
of peaks compared to a background set of sequences. It works by:

* Scanning each peak and matching background sequence for the motif
* Calculating a score for how well the motif fits each sequence
* Using statistical tests (e.g., Fisher’s exact test or rank-sum test) to see if
motif scores in peaks are higher than in background
* Reporting p-values for enrichment and adjusting them to control false
discoveries when testing many motifs

Since AME creates its background by shuffling your peaks (keeping dinucleotide
frequencies), each dataset has its own background model. The adjusted p-values
are reliable for ranking motifs within a dataset but should not be directly
compared between different datasets.

Instead, to compare across datasets, look at each motif’s rank by p-value,
examine odds ratios or score differences, and consider the percentage of peaks
containing the motif.

More details on AME output: 
[AME Output Documentation](https://meme-suite.org/meme/doc/ame-output-format.html)
